...
public interface SubjectRepository extends JpaRepository<Subject, String> {
    @Query("select new id.smartpesantren.dto.SubjectDTO(a) " +
            "from Subject a \n" +
            "where a.foundation.id=?#{principal.foundationId} \n" +
            "and upper(a.name) like :q")
    public Page<SubjectDTO> filter(@Param("q") String q, Pageable p);

    @Query("select new id.smartpesantren.dto.SubjectDTO(a) " +
            "from Subject a \n" +
            "where a.foundation.id=?#{principal.foundationId} \n" +
            "and upper(a.name) like :q")
    public Iterable<SubjectDTO> filterAll(@Param("q") String q);
}
